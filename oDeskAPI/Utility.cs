...
namespace oDeskAPI {
    public static class Utility {
        ...
        /// <summary>
        ///  Calculates the api_sig param for non-OAuth requests
        /// </summary>
        /// <param name="vals"></param>
        /// <param name="sharedSecret"></param>
        /// <returns></returns>
        public static string CalcApiSig(Dictionary<string, string> vals, string sharedSecret)
        {
            // Sort your argument list into alphabetical order based on the parameter name (e.g. foo=john&bar=doe sorts to bar=doe, foo=john)
            // Concatenate the shared secret and argument name-value pairs e.g. SECRETbardoefoojohn
            // Calculate the md5() hash of this string
            // Append this value to the argument list with the name api_sig, in hexidecimal string form e.g. api_sig=68cd0b2764cf0c40a62de5bfc21e58e5

            var ordered = vals.OrderBy(x => x.Key);
            var concatOrderedParams = String.Concat(
                ordered.Select(x => String.Concat(x.Key, x.Value)));

            var result = String.Concat(sharedSecret, concatOrderedParams);
		...
		public static IEnumerable<long> GetIndividualFlags(Enum flags) {
